	...
	for (uint i = 0; i < MAX_ENEMIES; ++i)
	{
		if (enemies[i] != nullptr && enemies[i]->GetCollider() == c1)
		{
			...
			if (enemies[i]->hp <= 0)
			{
				...
				{
					App->gexplosion->AddGroundExplosion(App->gexplosion->shiptank_explosion, enemies[i]->position.x, enemies[i]->position.y, COLLIDER_NONE);
					App->particles->AddParticle(App->particles->enemyshot, enemies[i]->position.x+ enemies[i]->w*3/2, enemies[i]->position.y + enemies[i]->h * 3/2, COLLIDER_NONE, 0,10,0 );


					App->particles->AddParticle(App->particles->enemyshot, enemies[i]->position.x + enemies[i]->w * 3 / 2, enemies[i]->position.y + enemies[i]->h * 3 / 2, COLLIDER_NONE, 0, 8, -5);
					App->particles->AddParticle(App->particles->enemyshot, enemies[i]->position.x + enemies[i]->w * 3 / 2, enemies[i]->position.y + enemies[i]->h * 3 / 2, COLLIDER_NONE, 0, 5, -8);


					App->particles->AddParticle(App->particles->enemyshot, enemies[i]->position.x + enemies[i]->w * 3 / 2, enemies[i]->position.y + enemies[i]->h * 3 / 2, COLLIDER_NONE, 0,  0,10);
