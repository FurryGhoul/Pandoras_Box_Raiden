#include "Enemy_Bonus_Ship.h"
#include "Application.h"
#include "ModulePlayer.h"
#include "ModuleCollision.h"
#include "ModuleParticles.h"
#include <Math.h>



Enemy_Bonus_Ship::Enemy_Bonus_Ship(int x, int y) : Enemy (x, y)
{
	flydown.PushBack({ 6, 23, 54, 52 });

	stop.PushBack({ 67, 23, 54, 52 });
	stop.PushBack({ 262, 23, 54, 52 });
	//stop.speed = 0.04;

	flysides.PushBack({ 133, 23, 59, 54 });
	flysides.PushBack({ 199, 23, 59, 54 });
	//flysides.speed = 0.2f;
	//flysides.loop = true;


	movement.PushBack({ 0.0f, 1.5f }, 190);
	movement.PushBack({ 0.0f, 0.0f }, 50);
	movement.PushBack({ -0.5f, 0.0f }, 300);
	movement.PushBack({ 0.5f, 0.0f }, 600);
	movement.PushBack({ -0.5f, 0.0f }, 600);
	movement.PushBack({ 0.5f, 0.0f }, 600);


	collider = App->collision->AddCollider({ 0, 0, 54*3, 52*3 }, COLLIDER_TYPE::COLLIDER_ENEMY, (Module*)App->enemies);
	spriteshit = 1;
	animations = 1;
	original_pos.x = x;
	original_pos.y = - 50;
}

void Enemy_Bonus_Ship::MoveShoot()
{
	position = original_pos + movement.GetCurrentPosition();
	animation = &flydown;
	w = 54 * 3;
	h = 52 * 3;
	if (position.x < 410 || position.x > 410 )
	{
		animation = &flysides;
		side = true;
	}
	if (position.y > 160 && position.x == 410 && side == false)
	{
		animation = &stop;
	}
	
	
	
}


